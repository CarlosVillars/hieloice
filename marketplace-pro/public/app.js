  ...
  viewEl.innerHTML = `
    ...
    <div class="product-detail">
      <div>
        ${
          mainPhoto
            ...
            : `<div class="product-thumb-empty" style="aspect-ratio:4/3;border-radius:10px;">📦</div>`
        }
        ${
          photos.length > 1
      ...
      <div class="detail-panel">
        ...
        <div class="detail-meta">📍 ${escapeHtml(locationLabel(p)) || "-"} &middot; ${fmtDate(p.createdAt)}</div>
        <div>
  ...
  viewEl.innerHTML = `
    <div class="profile-header">
      ...
      <div>
        ...
        ${profile.location ? `<p class="profile-sub">📍 ${escapeHtml(profile.location)}</p>` : ""}
          ...
          products.length
            ? products
                .map(
                  (p) => `
          <a class="product-card" href="#/product/${p.id}">
            ${p.photos && p.photos[0] ? `<img class="product-thumb" src="${p.photos[0]}" />` : `<div class="product-thumb-empty">📦</div>`}
            <div class="product-card-body">
              <p class="product-title">${escapeHtml(p.title)}</p>
              <p class="product-price">${fmtPrice(p.price)}</p>
            </div>
