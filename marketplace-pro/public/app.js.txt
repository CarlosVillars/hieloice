...
// Fill these in with the real profile URLs once each account exists.
// Empty string = badge shows but is not yet clickable (no live account yet).
const SOCIAL_LINKS = {
  facebook: "",
  instagram: "",
  whatsapp: "",
...
function applySocialLinks() {
  Object.keys(SOCIAL_LINKS).forEach((key) => {
    ...
    if (url) {
      ...
      el.classList.remove("footer-social-pending");
    } else {
      el.removeAttribute("href");
      el.classList.add("footer-social-pending");
    }
  });
}
...
function applyStaticI18n() {
  ...
  const socialLabelEl = document.getElementById("footer-social-label");
  if (socialLabelEl) socialLabelEl.textContent = I18N.t("footer.followUs");
}
