...
## 0. Standing operating instruction — tools & connectors

Set explicitly by Carlos on 2026-08-17, applies to every AI session on this
project going forward:

Proactively encourage the use of connected tools/integrations relevant to
whatever we're working on (GitHub, Stripe, Supabase, Figma, PostHog, Ahrefs,
Notion, etc.) — don't wait to be asked. Each time one is relevant, briefly
say what it does and what we'd use it for in this specific moment, then
**always ask permission before actually invoking/connecting it** — never
connect or call a new integration silently.

Quick reference on the ones already discussed:
- **GitHub — NOT connected** (checked 2026-08-17, confirmed via Carlos's own
  connectors screen: Notion/Figma/Stripe/Ahrefs/PostHog show "Connected",
  GitHub isn't listed at all). It doesn't live in the same simple personal
  connectors panel as the others - it's gated behind team/admin-level setup
  and isn't something to keep re-suggesting the same way. Until it's
  connected, deploys stay manual: after every code change, always send both
  the GitHub upload link and the Render dashboard link (see below).
- **Stripe** — connected. Payment processing; needed for the pending
  escrow/commission feature (book exchange payment protection) - but that
  feature itself is paused (Carlos has no LLC yet to accept card payments).
- **Supabase** — connected. Direct DB/migration/query access to the
  HieloIce database without going through server.js code changes.
- **Figma** — connected. For designing screens before building them, if
  that workflow is ever wanted.
- **PostHog** — connected. Product analytics (what people actually
  use/abandon — Moments vs. Marketplace, listing-flow drop-off).
- **Ahrefs** — connected. SEO, so people find HieloIce searching for used
  books.
- **Notion** — connected. Alternative/companion place for project docs.

## 0b. Standing operating instruction — deploy links

After **every** code change (any edit to files under `marketplace-pro/`),
always send both links, every time, without being asked:
- GitHub upload: `https://github.com/CarlosVillars/hieloice/upload/main/marketplace-pro/public`
  (or `.../marketplace-pro` for `server.js`)
- Render dashboard: `https://dashboard.render.com`

Also always be explicit about exactly which file(s) changed, since Carlos
only needs to re-upload the ones that actually changed, not the whole set.
- **Notion** — alternative/companion place for project docs, if preferred
  over this file.
